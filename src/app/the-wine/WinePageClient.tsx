  ...
  return (
    <div style={{ backgroundColor: 'var(--red)', overflowX: 'hidden' }}>
      ...
      <section style={{
        ...
        <div style={{ width: '100%', display: 'flex', padding: '0 clamp(7px, 0.81vw, 14px)', marginTop: 'clamp(8px, 1.5svh, 32px)', flex: 1, minHeight: 0, height: '55vw' }}>
          {PRODUCTS.map((item, i) => (
            <div key={item.name} style={{ flex: 1, display: 'flex', flexDirection: 'column', marginLeft: i > 0 ? '-2.22px' : 0, minHeight: 0 }}>
              <div style={{
                ...
                {i === 3 ? (
                  ...
                  <div style={{ position: 'absolute', bottom: 'clamp(12px, 2.03vw, 35px)', left: '50%', transform: 'translateX(-50%) translateY(7.5%)', width: '31.5vw', pointerEvents: 'none' }}>
                    <img src={item.img} alt={item.name} style={{ width: '100%', height: 'auto', display: 'block' }} />
                  </div>
                ) : (
                  <img src={item.img} alt={item.name} style={{ width: '42.7%', height: 'auto', display: 'block', marginBottom: '-5.2svh' }} />
                )}
              </div>
              <div style={{
